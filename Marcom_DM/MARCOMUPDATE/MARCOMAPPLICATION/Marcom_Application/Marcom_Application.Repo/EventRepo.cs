...
namespace Marcom_Application.Repo
{
    public class EventRepo
    {
        ...
        public static string UpdateRejectReason(VMEvent dataevent)
        {
            try
            {
                transaksi_event eventt = new transaksi_event();
                using (db_marcomEntities db = new db_marcomEntities())
                {
                    ...
                    eventt.status = 0;
                    eventt.reject_reason = dataevent.reject_reason;
        ...
        ////////closed
        public static string ClosedRequest(int id)
        {
            try
            {
                transaksi_event dataevent = new transaksi_event();
                using (db_marcomEntities db = new db_marcomEntities())
                {
                    dataevent = db.transaksi_event.Where(a => a.id == id).FirstOrDefault();
                    dataevent.status = 3;
                    dataevent.closed_date = System.DateTime.Now;

                    db.Entry(dataevent).State = System.Data.Entity.EntityState.Modified;
                    db.SaveChanges();
                }
                return "Berhasil" + dataevent.code;
            }
            catch (Exception e)
            {
